...
/* Perfil/config/contexto da loja (mesa de QR code, cupom pre-preenchido,
   nome, dados de contato/endereco, formas de pagamento, horarios, status
   aberto/fechado etc.) — montagem extraida para helpers/loja_perfil.php,
   compartilhada com o endpoint JSON publico (public/api/loja_perfil.php)
   usado pela Store no Next. Repoe as mesmas variaveis locais de sempre
   (nomeLoja, lojaAberta, capaLoja, ...) via extract(), sem mudanca de
   comportamento pro resto desta pagina. */
extract(montarPerfilLoja($conn, $lojaId));
...
/* Redirect pra URL canonica com ?loja=<slug> (slug/lojaCanonicalUrl ja
   vieram prontos do montarPerfilLoja() acima) */
...
/* Categorias/produtos/combos/destaques — montagem extraida para
   helpers/loja_catalogo.php, compartilhada com o endpoint JSON publico
   (public/api/loja_catalogo.php) usado pela Store no Next. */
extract(montarCatalogoLoja($conn, $lojaId));
