...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await phpApiFetch(
      `/admin/api/v1/cliente_pedidos.php?cliente_id=${encodeURIComponent(id)}&periodo=${encodeURIComponent(periodo)}&tipo=${encodeURIComponent(tipo)}&pagina=${encodeURIComponent(pagina)}`
    );
