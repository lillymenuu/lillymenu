...
function calcularTaxaDinamica(PDO $conn, float $distancia, int $lojaId, bool $porKm = false): float {
...
if ($tipoTaxa === 'bairro' && tabelaExiste($conn, 'taxas_bairro') && !empty($destino['bairro'])) {
  ...
  if ($temDinamica) {
    $taxaEntrega = calcularTaxaDinamica($conn, $distancia, $lojaId, $tipoTaxa === 'area');
  }
}
