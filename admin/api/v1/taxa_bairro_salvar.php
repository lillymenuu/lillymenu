<?php
/*
 * Versao JSON (Bearer token) de admin/api/taxa_bairro_save.php — porta 1:1.
 */
...
if ($bairro === '' || $taxa < 0) {
  echo json_encode(['ok' => false, 'msg' => 'Informe bairro e taxa validos.'], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
  exit;
}
...
try {
  ...
  if ($stmtDup->fetchColumn()) {
    echo json_encode(['ok' => false, 'msg' => 'Ja existe uma taxa cadastrada para este bairro.'], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    exit;
  }
  if ($id > 0) {
    $stmt = $conn->prepare("
