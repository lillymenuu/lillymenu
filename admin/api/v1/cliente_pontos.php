<?php
/*
 * Versao JSON de admin/api/cliente_pontos.php para o novo frontend
 * Next.js — aba "Historico de pontos" do modal de cliente.
 */
...
function clientePontosTabelaExiste(PDO $conn, string $tabela): bool {
  $stmt = $conn->prepare("SHOW TABLES LIKE ?");
  $stmt->execute([$tabela]);
  return (bool) $stmt->fetchColumn();
}

if (!clientePontosTabelaExiste($conn, 'pontos_movimentacoes')) {
  echo json_encode(['ok' => true, 'pagina' => 1, 'paginas' => 1, 'total' => 0, 'pontos' => []]);
  exit;
}
