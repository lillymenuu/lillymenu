...
const receberPedidosWhatsappInput = document.getElementById('receberPedidosWhatsapp');
if (receberPedidosWhatsappInput) {
  const maskWhatsappNumero = el => {
    let v = el.value.replace(/\D/g, '').slice(0, 11);
    if (v.length > 10) {
      v = v.replace(/(\d{2})(\d{5})(\d{0,4})/, '($1) $2-$3');
    } else if (v.length > 5) {
  ...
  receberPedidosWhatsappInput.addEventListener('input', () => maskWhatsappNumero(receberPedidosWhatsappInput));
  maskWhatsappNumero(receberPedidosWhatsappInput);
}
